...
writer_loc ='/home/chaimeleon/persistent-home/classificationmodel_harmonizednew/axial_harmonize/rectum_centrecrop2/logs_FM'
save_model_loc = '/home/chaimeleon/persistent-home/classificationmodel_harmonizednew/axial_harmonize/rectum_centrecrop2/models_FM'
csv_location = '/home/chaimeleon/persistent-home/Complete_set/Experiments/Results/axial_harmo_FM192'

# model saving directory
train_dir = r"/home/chaimeleon/persistent-home/classificationmodel_harmonizednew/axial_harmonize/rectum_centrecrop2/train"
val_dir = r"/home/chaimeleon/persistent-home/classificationmodel_harmonizednew/axial_harmonize/rectum_centrecrop2/val"
    
train_label = r'/home/chaimeleon/persistent-home/classificationmodel_harmonizednew/axial_harmonize/rectum_centrecrop2/train_labels.csv'
val_label = r'/home/chaimeleon/persistent-home/classificationmodel_harmonizednew/axial_harmonize/rectum_centrecrop2/val_labels.csv'
...
Prefix = 'FM_axial_harmo' #Increased Pos weight
# prior_name ='gland'
...
accumulation_steps =24 #32*8=256 img size
#EXTRA PRECAUTION
...
def SetupModel(device):
    ...
    print("fmEncoder is printing now....")
    print(fmEncoder)
...
def run_gradcam_on_tensor(img_tensor, fm_encoder, class_index, save_path):
    input_tensor = img_tensor.unsqueeze(0).to(device)

    swin_encoder = SwinEncoderWrapper(fm_encoder["encoder"]).to(device)
    target_layer = swin_encoder.encoder.features[7][0].mlp[3]

    cam = GradCAM(model=swin_encoder, target_layers=[target_layer], use_cuda=True)
    grayscale_cam = cam(input_tensor=input_tensor, targets=[ClassifierOutputTarget(class_index)])[0]

    slice_rgb = img_tensor.cpu().numpy().transpose(1,2,0)
    slice_rgb = (slice_rgb - slice_rgb.min()) / (slice_rgb.max() - slice_rgb.min() + 1e-5)
    cam_image = show_cam_on_image(slice_rgb, grayscale_cam, use_rgb=True)
...
def validation(model1,model2,fmEncoder,val_loader,device,loss_function1,loss_function2, csv_prefix=''):
    ...
    with torch.no_grad():
        for batch in val_loader:
            img = batch['image'].to(device, non_blocking=True) #[B,d,c,h,w]
            evi_labels = batch['label']['EVI'].to(device,dtype=torch.float,non_blocking=True)
            mfi_labels = batch['label']['MFI'].to(device,dtype=torch.float,non_blocking=True)
            ...
            with torch.autocast(device_type='cuda'):
                ...
                with torch.no_grad():
                    ...
                    if len(all_output_evi) == 0:
                        run_gradcam_on_tensor(
                            img_tensor=img[0],
                            fm_encoder=fmEncoder,
                            class_index=int(evi_labels[0].item()),
                            save_path=f"/home/chaimeleon/persistent-home/Complete_set/gradcam/gradcam_evi_0.png"
                        )

                    o1, weights1 = model1(hidden_vector,supercase_indices)
                    o2, weights1 = model2(hidden_vector,supercase_indices)
...
def RunInference():
        ...
        val_dir = val_dir.replace('/val','/test')
        val_label = val_label.replace('val_labels.csv','test_labels.csv')
        ...
        val_loss1,val_loss2, metrics_evi, metrics_mfi = validation(model1,model2,fmEncoder,val_loader,device,loss_function1,loss_function2,csv_prefix='best')
